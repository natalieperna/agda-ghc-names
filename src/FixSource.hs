...
-- transform a *.hs file into *.agdaIdents.hs


module FixSource where
...
updateSource :: (String -> String -> Maybe String) -> Bool -> Bool -> [String] -> [String]
updateSource resolve keepMod keepSrc [] = []
updateSource resolve keepMod keepSrc (s : ss) = case checkSepLine s of
  Nothing -> s : updateSource resolve keepMod keepSrc ss
  Just (ccWidth, modWidth, mSrcWidth) -> let
   ...
   :   new
   ++  []
   :   updateSource resolve keepMod keepSrc rest

updateSourceFile  :: IO ()
                -> (String -> String -> Maybe String)
                -> Bool -> Bool -> FilePath -> IO ()
updateSourceFile usage resolve keepMod keepSrc path
  = case stripPrefix (reverse ".hs") (reverse path) of
  Nothing -> usage
  Just revBasename -> let
    ...
    writeFile path' . unlines . updateSource resolve keepMod keepSrc $ lines s
